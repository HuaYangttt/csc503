: - consult('swi-prolog.pl').
:- consult('phdCanGraduate.pl').
:- consult('phdTerminated.pl').
:- consult('whyCanOrCannotTake.pl').
:- consult('phdRecommend.pl').

% registration history
registrationSemester(s004, phd, fall, 2022, yes).
registrationSemester(s004, phd, spring, 2023, no).
registrationSemester(s004, phd, fall, 2023, no).
registrationSemester(s004, phd, spring, 2024, no).
registrationSemester(s004, phd, fall, 2024, no).
registrationSemester(s004, phd, spring, 2025, no).
registrationSemester(s004, phd, fall, 2025, no).

% Fall 2022 (10 credits) - First semester
% Orientation course (S/U)
hasTakenCourse(s004, csc600, s001, 1, 3.0).
% Core - Theory
hasTakenCourse(s004, csc505, s001, 3, 4.0).  
hasTakenCourse(s004, csc503, s001, 3, 4.0).
% Core - Systems
hasTakenCourse(s004, csc501, s001, 3, 4.0).  

% Spring 2023 (9 credits)
% Core - Systems
hasTakenCourse(s004, csc540, s001, 3, 4.0).
% 700-level courses
hasTakenCourse(s004, csc714, s001, 3, 4.0).  
hasTakenCourse(s004, csc712, s001, 3, 4.0). 

% Fall 2023 (12 credits)
% Written Prelim (S/U)
hasTakenCourse(s004, csc890, s001, 3, 3.0).
% Electives
hasTakenCourse(s004, csc520, s001, 3, 4.0).  
hasTakenCourse(s004, csc570, s001, 3, 4.0).

% Spring 2024 (9 credits)
% Electives
hasTakenCourse(s004, csc574, s001, 3, 4.0).   
hasTakenCourse(s004, csc565, s001, 3, 4.0).
% Oral Prelim (S/U)
hasTakenCourse(s004, csc890, s001, 3, 3.0).

% Fall 2024 (9 credits)
% Research credits (S/U)
hasTakenCourse(s004, csc893, s001, 6, 3.0).
hasTakenCourse(s004, csc561, s001, 3, 4.0).

% Spring 2025 (9 credits)
% Research credits (S/U)
hasTakenCourse(s004, csc893, s001, 9, 3.0).

% Exam outcomes
phdWrittenExamTaken(s004, fall, 2023, 3.0).
phdOralExamTaken(s004, spring, 2024, 3.0).

% Graduate advisor
graduateAdvisor(s004, advisor1, phd).

% Advisory committee (4 members)
advisoryCommitteeMember(s004, advisor1).
advisoryCommitteeMember(s004, member2).
advisoryCommitteeMember(s004, member3).
advisoryCommitteeMember(s004, member4).

% Faculty affiliations
facultyAffiliation(advisor1, csc).
facultyAffiliation(member2, csc).
facultyAffiliation(member3, ece).  % Outside CS
facultyAffiliation(member4, csc).

% Plan approved
planOfGraduateWorkApproved(s004).